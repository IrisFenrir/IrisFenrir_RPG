            ...
            foreach (var data in asset.keys)
            {
                switch(data.type)
                {
                    ...
                    case KeyDataUnit.KeyType.AxisKey:
                        int axisCount = data.subKeys.Count;
                        AxisKey axisKey = new AxisKey(axisCount);
                        axisKey.name = data.GetStringProperty("name");
                        for (int i = 0; i < axisCount; i++)
                        {
                            KeyDataUnit subKey = data.subKeys[i];
                            axisKey.AddAxis(i, subKey.GetKeyCodeProperty("posKey"), subKey.GetKeyCodeProperty("negKey"), subKey.GetVector2Property("range"), subKey.GetFloatProperty("start"), subKey.GetVector2Property("posSpeed"), subKey.GetVector2Property("negSpeed"));
                        }
